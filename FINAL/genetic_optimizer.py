...
class GeneticOptimizer:
    ...
    def _fitness(self, individuo):
        ...
        for i in range(len(self.productos)):
            for j in range(i + 1, len(self.productos)):
                ...
                distancia = self.almacen._calcular_distancia_manhattan(
    ...
    def _crossover(self, padre1, padre2):
        ...
        hijo = {}
        # crossover simple (se puede mejorar para evitar duplicados de forma mas elegante)
        p1_genes = list(padre1.items())
        p2_genes = list(padre2.items())
        hijo_genes = p1_genes[:punto] + p2_genes[punto:]
        hijo = dict(hijo_genes)
        # simple correccion de duplicados
        if len(set(hijo.values())) < len(hijo):
            return self._crear_individuo()
        return hijo
